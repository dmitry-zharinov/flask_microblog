...
# вспомогательная таблица followers
...
class User(UserMixin, db.Model):
    ...
    def followed_posts(self):
        # запрос Post.query.join(...).filter(...).order_by(...) - найти посты от фолловеров
        # 1) получить JOIN всех постов от пользователей за которыми кто-то следит
        # 2) Отфильтровать только те посты, которые относятся к фолловерам текущего пользователя
